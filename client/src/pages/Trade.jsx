...
const Trade = ({ symbol }) => {
  ...
  const handleTrade = async (type) => {
    try {
      if (!symbol) return toast.error("No stock selected");
      if (isIndex) return toast.error("Indexes cannot be traded");
      if (!quantity || quantity <= 0) return toast.error("Enter valid quantity");

      if (orderType !== "MARKET" && (!price || price <= 0)) {
        return toast.error("Enter a valid price for limit order");
      }
      ...
      toast.success(`${type} order executed: ${quantity} ${symbol}`);
